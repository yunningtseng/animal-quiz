...
const ANIMAL_PIC: { [key: string]: string } = {
  哺乳綱: 'Choloepus-didactylus',
  鳥綱: 'Ramphastos-toco',
  昆蟲綱: 'Ideopsis-similis',
};

const ANIMAL_TYPE: { [key: string]: string } = {
  哺乳綱: '哺乳類 / Mammals',
  鳥綱: '鳥類 / Birds',
  昆蟲綱: '昆蟲類 / Insects',
};

function AnimalCategory({ animalCategory }: Props) {
  return (
      ...
      <Link to={`/animals/${animalCategory}`}>
        <div className="flex flex-col mb-10 cursor-pointer">
          <div className="md:w-52 md:h-44 lg:w-72 lg:h-60 xl:w-96 xl:h-72 rounded-lg shadow-xl overflow-hidden">
            <img
              src={`${IMG_BASE_URL}${ANIMAL_PIC[animalCategory]}-01.jpg`}
              alt="img"
